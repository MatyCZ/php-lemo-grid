...
class GridFactory
{
    ...
    public function createAdapter($spec)
    {
        ...
        throw new Exception\DomainException(sprintf(
            ...
            __METHOD__,
            'Lemo\Grid\Adapter\AdapterInterface',
            $spec['type']
        ));
    }
    ...
    {
        $spec = $this->validateSpecification($spec, __METHOD__);
        $type = isset($spec['type']) ? $spec['type'] : 'Lemo\Grid\Column';
        ...
        throw new Exception\DomainException(sprintf(
            '%s expects the $spec["type"] to implement one of %s, %s, or %s; received %s',
            __METHOD__,
            'Lemo\Grid\Column\ColumnInterface',
            $spec['type']
        ));
    }
    ...
    public function createPlatform($spec)
    {
        ...
        throw new Exception\DomainException(sprintf(
            '%s expects the $spec["type"] to implement one of %s, %s, or %s; received %s',
            __METHOD__,
            'Lemo\Grid\Platform\PlatformInterface',
            $spec['type']
        ));
    }
    ...
    public function createStorage($spec)
    {
        ...
        throw new Exception\DomainException(sprintf(
            '%s expects the $spec["type"] to implement one of %s, %s, or %s; received %s',
            __METHOD__,
            'Lemo\Grid\Storage\StorageInterface',
            $spec['type']
        ));
    }
     ...
     *
     * Specification can contain any of the following:
     * - type: the Column class to use; defaults to \Lemo\Grid\Column
     ...
     *
     * Specification can contain any of the following:
     * - type: the Grid class to use; defaults to \Lemo\Grid\Grid
    ...
    protected function prepareAndInjectAdapter($adapterOrName, GridInterface $grid, $method)
    {
        ...
        if (!$adapter instanceof Adapter\AdapterInterface) {
            throw new Exception\DomainException(sprintf(
                '%s expects a valid implementation of Lemo\Grid\Adapter\AdapterInterface; received "%s"',
                $method,
                $adapterOrName
            ));
        }
    ...
    protected function prepareAndInjectPlatform($platformOrName, GridInterface $grid, $method)
    {
        ...
        if (!$platform instanceof Platform\PlatformInterface) {
            throw new Exception\DomainException(sprintf(
                '%s expects a valid implementation of Lemo\Grid\Platform\PlatformInterface; received "%s"',
                $method,
                $platformOrName
            ));
        }
    ...
    protected function prepareAndInjectStorage($storageOrName, GridInterface $grid, $method)
    {
        ...
        if (!$storage instanceof Storage\StorageInterface) {
            throw new Exception\DomainException(sprintf(
                '%s expects a valid implementation of Lemo\Grid\Storage\StorageInterface; received "%s"',
                $method,
                $storageOrName
            ));
        }
