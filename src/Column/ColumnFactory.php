 ...
 */
final class ColumnFactory implements FactoryInterface
{
    /**
     ...
     */
    public function createService(ServiceLocatorInterface $containerInterface, $canonicalName = null, $requestedName = null)
    {
        if (class_exists($canonicalName)) {
            return $this($containerInterface, $canonicalName, $this->creationOptions);
        }

        if (is_string($requestedName) && class_exists($requestedName)) {
            return $this($containerInterface, $requestedName, $this->creationOptions);
        }
